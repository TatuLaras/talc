...
int str_to_symbols_postfix(char *src_str, SymbolStack *output_stack) {
    ...
    for (size_t i = 0; src_str[i] != 0; i++) {
        ...
        if (src_str[i] == '+' || src_str[i] == '-' || src_str[i] == '*' ||
            ...
            push_symbol_with_shunting_yard(op, output_stack, &holding_stack);
            continue;
        }
    ...
    drain_holding_stack_into_output(output_stack, &holding_stack);
    return 0;
}
...
static int push_symbol_with_shunting_yard(Symbol symbol,
    ...
    if (symbol.symbol_type == SYMBOL_LITERAL_INTEGER ||
        ...
        stack_push(output_stack, symbol);
        return 0;
    }
    ...
    while (holding_stack->__top > 0 &&
        ...
        stack_push(output_stack, top_symbol);
        stack_peek_from_top(holding_stack, 0, &top_symbol);
    }
...
int drain_holding_stack_into_output(SymbolStack *output_stack,
    ...
    while (holding_stack->__top > 0) {
        ...
        stack_push(output_stack, out_symbol);
    }
